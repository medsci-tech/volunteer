    ...
    <div file-upload-module style="padding-left: 20px;padding-top: 20px;">
        ...
        <div style="padding: 5px;">
            <button class="btn btn-default" style="position: relative;overflow: hidden;display: inline-block;">选择文件
                <input name="excel" type="file" id="fileupload" data-url="{{url('/admin/hospital/import')}}" data-form-data='{"_token": "{{csrf_token()}}"}'>
            </button>
        ...
        <div style="padding: 5px;"><button class="btn btn-default" onclick="importFile();">导入</button>
            <a href="{{url('/admin/hospital/sampleExcel')}}">下载表格模板</a>
        </div>
    </div>
@endsection
            ...
            $('#fileupload').fileupload({
                ...
                success:function (result, textStatus, jqXHR){
                    if(result &&result.success){
                        ...
                        $('div[file-upload-module] span[file-name]').text(result.msg);
                    }
                },
                error:function(){
